...
- **Append, never replace:** `force_2fa_filter_enabled_providers()` (hooked to Two Factor's `two_factor_enabled_providers_for_user` filter) appends the Email provider to an in-scope user's list. Users' stronger factors (TOTP, WebAuthn) and backup codes must always be preserved. Email stays a floor for everyone in scope — including users who already have a native method. **External 2FA exemption:** users whose 2FA is handled by Wordfence (bundled defensive `force_2fa_wordfence_2fa_active()` check — the only built-in integration; other systems use the `force_2fa_user_is_exempt` filter) are marked exempt in `force_2fa_compute_exemption()`, so the existing "exempt → don't append" path skips the floor. **Primary provider:** `force_2fa_filter_primary_provider()` (Two Factor's `two_factor_primary_provider_for_user`) makes Email primary only when the user has no *real* method — `force_2fa_is_real_2fa_provider_key()` (single source of truth) excludes Email, Backup Codes, and the debug-only `Two_Factor_Dummy`. A real method always outranks the email floor; stored primary meta is never mutated.
- **Enforcement scope, default all users:** `force_2fa_user_is_exempt()` delegates to the pure `force_2fa_exemption_decision()`. `FORCE_2FA_ENFORCED_CAPABILITY` is an opt-in gate — when set (e.g. `manage_options`), users lacking that capability are exempt. `force_2fa_enforced_capability()` precedence: **wp-config constant → stored first-run choice (`force_2fa_scope_choice_get()`) → `''` (everyone)**, then the filter. The first-run notice + `admin_post` handler store the choice; a defined constant suppresses the prompt; deactivation clears it (re-prompt). On multisite both the capability check and the `FORCE_2FA_EXCLUDED_ROLES` carve-out are evaluated **network-wide** (super admin, or the capability/role on any of the user's sites), since WordPress logins are network-wide — a per-login-site check would be a cross-site bypass. **The XML-RPC API-login allowlist is scope-independent:** `force_2fa_gate_api_login()` (on `authenticate`) holds every XML-RPC login to the allowlist + app-password policy for ALL users, so narrowing the scope (or excluding a role) never opens XML-RPC to out-of-scope accounts (governs XML-RPC, not REST).
- **API logins:** an XML-RPC login is allowed only when the account is on the explicit allowlist **and** the request authenticated with an Application Password (tracked via `force_2fa_note_app_password_user()`); everything else on the XML-RPC path is denied. Enforced by two hooks with the same policy: `force_2fa_filter_api_login_enable()` (Two Factor's `two_factor_user_api_login_enable`, in-scope users) **and** `force_2fa_gate_api_login()` (own `authenticate` gate) which extends it to ALL users, independent of the 2FA scope. Pure decision: `force_2fa_api_login_should_deny()`. REST is not gated.
