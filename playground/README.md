...
- Codes expire after 15 minutes. Two Factor fails **silently** on an expired or
  wrong code — the challenge page just re-appears with no error. On that reload it
  auto-issues a fresh code (the helper updates the on-screen value), or you can
  click **Resend Code**; use the newly shown code.
