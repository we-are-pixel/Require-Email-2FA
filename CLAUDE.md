...
- **Enforcement scope, default all users:** `force_2fa_user_is_exempt()` delegates to the pure `force_2fa_exemption_decision()`. `FORCE_2FA_ENFORCED_CAPABILITY` (read via `defined()`, **default `''` = everyone**) is an opt-in gate — when set (e.g. `manage_options`), users lacking that capability are exempt. The capability check (`force_2fa_user_has_capability()`) is network-aware: super admins always in scope, evaluated per current site via `user_can_for_site()` (WP 6.7+) with a `user_can()` fallback for 6.5–6.6. `FORCE_2FA_EXCLUDED_ROLES` is a further role carve-out. **Narrowing the scope also narrows the API-login hardening** (Two Factor only gates API logins for users "using 2FA"), the same trade-off as excluded roles — keep the default `''` unless that's acceptable.
