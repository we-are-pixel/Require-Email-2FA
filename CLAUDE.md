...
- **Append, never replace:** `force_2fa_filter_enabled_providers()` (hooked to Two Factor's `two_factor_enabled_providers_for_user` filter) appends the Email provider to an in-scope user's list. Users' stronger factors (TOTP, WebAuthn) and backup codes must always be preserved. Email stays a floor for everyone in scope — including users who already have a native method. **External 2FA exemption:** users whose 2FA is handled outside Two Factor (bundled defensive Wordfence check via `force_2fa_wordfence_2fa_active()`, generalized through the `force_2fa_user_has_external_2fa` filter) are marked exempt in `force_2fa_compute_exemption()`, so the existing "exempt → don't append" path skips the floor (never route a user through two 2FA plugins). **Primary provider:** `force_2fa_filter_primary_provider()` (Two Factor's `two_factor_primary_provider_for_user`) makes Email primary only when the user has no *real* method (available, non-Email, non-Backup-Codes) — a real method always outranks the email floor; stored primary meta is never mutated.
